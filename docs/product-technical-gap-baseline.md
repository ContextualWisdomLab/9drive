...
| P0 | Exact-head security intake is not GREEN | Predecessor head `ed9248ec…` produced three Semgrep errors: missing non-root users in both runtime images and an AES-GCM decrypt path without an explicit tag length. The same scan also reported vulnerable backend/frontend lockfile packages; Dependency Review returned HTTP 403 before analysis. | Keep the source regressions executable, verify both images run as non-root, require a 128-bit GCM tag, then upgrade/replace vulnerable dependency paths and obtain unchanged-head Semgrep, Trivy, OSV, Dependency Review and CodeQL GREEN without suppression | **Source repair candidate on #3; dependency remediation open** |
