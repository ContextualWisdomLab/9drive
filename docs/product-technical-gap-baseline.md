...
Application-held provider tokens use AES-256-GCM with a 96-bit random IV and a required 128-bit authentication tag. Both runtime images declare non-root users. The frontend Nginx process listens on container port 8080, while Compose retains the loopback-only user entry point at `127.0.0.1:5173`.

Google Drive uploads remain private by provider default. The canonical writer removes the previous upload-time `anyone/writer` permission creation from both synchronous and resumable upload paths. Public sharing, when explicitly requested through a product sharing flow, must remain a separate authorized and revocable action rather than a storage side effect. `backend/tests/storage-security-contracts.test.mjs` executes the request-body builder and verifies that upload metadata contains only the file name and parent folder rather than scanning implementation text.
...
Before changing an existing SQLite-based installation, the operator must stop the legacy backend, copy the complete Prisma directory (including any journal/WAL companions), require SQLite `PRAGMA integrity_check` to return `ok`, and record digests while the exact legacy container/revision is still available. The new MySQL Compose topology must not be started under the assumption that it imports that file. The branch also makes `/system/backup` and `/system/restore` fail closed with explicit unsupported responses under non-file databases instead of interpreting a MySQL URL as a filesystem path. The storage security contract executes the shared database-URL/path decision for SQLite, MySQL, and PostgreSQL inputs rather than counting guards in source text. A validated SQLite-to-MySQL conversion plus MySQL backup/restore acceptance remains required before this is an in-place upgrade path.
...
| P0 | Legacy persistence migration and MySQL recovery are incomplete | Predecessor Compose named a SQLite file/volume while current Prisma schema is MySQL; a running single-file copy can omit journal/WAL state, and no validated SQLite-to-MySQL converter or MySQL backup/restore acceptance exists | Keep MySQL Compose new-install-only, quiesce and integrity-check a complete legacy DB directory before revision change, fail closed for file-only backup APIs on MySQL, then implement and verify conversion plus MySQL backup/restore/recovery before in-place upgrade claims | **In progress on #3; migration/recovery open** |
...
| P0 | Exact-head security intake is not GREEN | Predecessor head `ed9248ec…` produced three Semgrep errors: missing non-root users in both runtime images and an AES-GCM decrypt path without an explicit tag length. The same scan also reported vulnerable backend/frontend lockfile packages; Dependency Review returned HTTP 403 before analysis. After the source and compatible lock repairs, local production audit is 0 for frontend and still 3 HIGH for the Prisma 6.19.3 → `@prisma/config` → `deepmerge-ts` chain. npm proposes an out-of-range Prisma downgrade, while current upstream stable requires a major transition; neither is accepted without compatibility evidence. | Keep the source regressions executable, verify both images run as non-root, require a 128-bit GCM tag, and validate a canonical Prisma/deepmerge upgrade or replacement with schema/migration/build/runtime evidence; then obtain unchanged-head Semgrep, Trivy, OSV, Dependency Review and CodeQL GREEN without suppression | **Source/direct-compatible repair candidate on #3; Prisma owner transition open** |
...
| P1 | Google uploads previously broadened provider ACL during storage | Synchronous and resumable paths created `anyone/writer` permissions as an upload side effect | Keep provider objects private by default; public sharing must be explicit, authorized, least-privilege and revocable | **Candidate repair + regression on #3; exact-head verification required** |
| P1 | Locked source-install contract was absent from onboarding | Both workspaces have committed lockfiles but prior README used mutable `npm install` | Use `npm ci` for backend/frontend source development and verification; preserve lockfile-backed dependency identity | **Candidate repair on #3; exact-head verification required** |
