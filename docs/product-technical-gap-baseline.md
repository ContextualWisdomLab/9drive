...
`backend/package.json` still declares `ISC`. Apache-2.0 and ISC are both commercial-use-permissive, but the package/repository metadata mismatch is an inherited provenance-alignment question that must be resolved before a standalone backend-package publication. Third-party npm packages, Google APIs, S3 services, MySQL, container images, and external assets/services retain their own terms.
...
| P1 | Backend package license metadata differs from inherited repository grant | `backend/package.json` says ISC while root lineage is upstream Apache-2.0 | Trace upstream package provenance and intended backend distribution; align metadata only with rights-backed evidence and package/release tests | **Open provenance gap** |
